...
function openApp(appName) {
    ...
    if (appName === "music") {
        ...
        if (isControlPanelOpen) { 
            if (controlPanelWinBox) {
                if (controlPanelWinBox.min) controlPanelWinBox.restore();
                controlPanelWinBox.focus();
            }
            return; 
        }
        ...
        controlPanelWinBox = new WinBox({
            title: "Control Panel",
            icon: "https://img.icons8.com/fluency/48/000000/control-panel.png",
...
function closeApp(appName) {
    ...
    if (appName === "music") {
        ...
        if (controlPanelWinBox) controlPanelWinBox.close();
        return;
    } else if (appName === "live2d") {
        isLive2dOpen = false;
        appFrame = live2dWallpaperFrame;
...
function minimizeApp(appName) {
    ...
    if (appName === "music") {
        ...
        if (controlPanelWinBox) {
            if (!controlPanelWinBox.min) controlPanelWinBox.minimize();
        }
        return;
    } else if (appName === "live2d") {
        appFrame = live2dWallpaperFrame;
...
function maximizeApp(appName) {
    ...
    if (appName === "music") {
        ...
        if (controlPanelWinBox) {
            controlPanelWinBox.max ? controlPanelWinBox.restore() : controlPanelWinBox.maximize();
        }
        return;
    } else if (appName === "live2d") {
        appFrame = live2dWallpaperFrame;
...
function bringToFront(appName) {
    ...
    else if (appName === "cp") { if (controlPanelWinBox) controlPanelWinBox.focus(); }
    ...
    window.addEventListener("message", (event) => {
        ...
        switch (action) {
            ...
            case "request-settings":
                 if (controlPanelWinBox && controlPanelWinBox.window.querySelector('iframe')?.contentWindow) {
                     controlPanelWinBox.window.querySelector('iframe').contentWindow.postMessage({ action: "request-settings" }, "*");
                 }
                 break;
            ...
            case "toggle-fancy-mode":
                ...
                if (controlPanelWinBox && controlPanelWinBox.window.querySelector('iframe')?.contentWindow) {
                    controlPanelWinBox.window.querySelector('iframe').contentWindow.postMessage({ action: "toggle-fancy-mode", value: value }, "*");
                }
                [musicPlayerFrame, videoPlayerFrame, live2dWallpaperFrame, fileExplorerFrame].forEach(frame => {
            ...
            case "change-theme": {
                ...
                if (controlPanelWinBox) {
                    controlPanelWinBox.removeClass("win11-light").removeClass("win11-dark").addClass(isDark ? "win11-dark" : "win11-light");
                    if (controlPanelWinBox.window.querySelector('iframe')?.contentWindow) {
                        controlPanelWinBox.window.querySelector('iframe').contentWindow.postMessage({ action: "theme-change", isDark }, "*");
                    }
                }
                [musicPlayerFrame, videoPlayerFrame].forEach(frame => {
